...
export default class DocumentSigsM extends Component {
    ...
    edit = (id) => {
        ...
        Api.get('/document_sigs_horarios/dataForm', {id: id}, (rsp) => {
            if (rsp.ok) {
                this.setState({
                    loading: false,
    ...
    save = (e) => {
        ...
        Api.post('/document_sigs_horarios/create', formData, (rsp) => {
    ...
    remove = () => {
        if (window.confirm('¿Seguro que quieres borrar?')) {
            ...
            Api.get('/document_sigs_horarios/remove', {id: this.state.item.id}, (rsp) => {
                if (rsp.ok) {
            ...
            <Modal isOpen={this.state.modal} className={this.props.className} backdrop="static" keyboard={false}>
                <fieldset disabled={this.state.loading}>
                    <ModalHeader toggle={this.toggle}>
                        {this.state.item.id > 0 ? 'Editar' : 'Agregar'} Horarios
                    </ModalHeader>
