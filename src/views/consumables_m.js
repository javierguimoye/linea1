...
export default class ConsumablesM extends Component {
    ...
    edit = (id) => {
        ...
        Api.get('/consumables/dataForm', {id: id}, (rsp) => {
            if (rsp.ok) {
                this.setState({
                    loading: false,
    ...
    save = (e) => {
        ...
        Api.post('/consumables/create', this.state.item, (rsp) => {
    ...
    remove = () => {
        if (window.confirm('¿Seguro que quieres borrar?')) {
            ...
            Api.get('/consumables/remove', {id: this.state.item.id}, (rsp) => {
                if (rsp.ok) {
            ...
            <Modal isOpen={this.state.modal} className={this.props.className} backdrop="static" keyboard={false}>
                <fieldset disabled={this.state.loading}>
                    <ModalHeader toggle={this.toggle}>
                        {this.state.item.id > 0 ? 'Editar' : 'Agregar'} EPP
                    </ModalHeader>
