...
export default class CompetencesM extends Component {
    ...
    edit = (id) => {
        ...
        Api.get('/competences/dataForm', {id: id}, (rsp) => {
            if (rsp.ok) {
                this.setState({
                    loading: false,
    ...
    save = (e) => {
        ...
        Api.post('/competences/create', this.state.item, (rsp) => {
    ...
    remove = () => {
        if (window.confirm('¿Seguro que quieres borrar?')) {
            ...
            Api.get('/competences/remove', {id: this.state.item.id}, (rsp) => {
                if (rsp.ok) {
            ...
            <Modal isOpen={this.state.modal} className={this.props.className} backdrop="static" keyboard={false}>
                <fieldset disabled={this.state.loading}>
                    <ModalHeader toggle={this.toggle}>
                        {this.state.item.id > 0 ? 'Editar' : 'Agregar'} competencia
                    </ModalHeader>
